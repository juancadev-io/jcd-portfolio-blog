export const ui = {
  en: {
    "nav.home": "Home",
    "nav.about": "About",
    "nav.twitter": "Twitter",
    "site.title": "JuancaDev Portfolio Blog",
    "site.description":
      "Welcome to my portfolio blog where I share my projects and thoughts.",
    "site.update.date": "Jan 1 2024",
    "home.title": "Hi there",
    "home.description":
      "I’m Juancadev, a software developer focused on web development and game development with Godot. I create practical, hands-on content where I explain how things work behind the scenes, using real examples, clean code, and good practices, with the goal of helping other developers build strong foundations, develop technical thinking, and create their own projects with confidence.",
    "about.title": "About Me",
    "about.description":
      "Hi! I am JuancaDev, a passionate developer who loves creating web applications and sharing knowledge with the community.",
    "pagination.first": "First",
    "pagination.last": "Last",
    "pagination.prev": "Previous",
    "pagination.next": "Next",
    "page": "Page",
    "published.on": "Published on",
    "updated.on": "Updated on",
    "footer.love": "Made with ❤️ for the community by JuancaDev",
    "home.work.title": "Work",
    "home.work.current.name": "Dafiti",
    "home.work.current.description": "supporting the growth of Latin America's leading fashion e-commerce platform.",
    "home.work.previous.name": "Banco de Bogotá",
    "home.work.previous.description": "developing innovative financial solutions to enhance customer experience.",
    "home.work.last.name": "Canastto",
    "home.work.last.description": "building and maintaining robust backend services for a growing e-commerce platform.",
    "home.find.me": "Find me on",
  },
  es: {
    "nav.home": "Inicio",
    "nav.about": "Acerca de",
    "nav.twitter": "Twitter",
    "site.title": "JuancaDev Blog de Portafolio",
    "site.description":
      "Bienvenido a mi blog de portafolio donde comparto mis proyectos y pensamientos.",
    "site.update.date": "1 Ene 2024",
    "home.title": "Hola",
    "home.description":
      "Soy Juancadev, desarrollador de software enfocado en desarrollo web y desarrollo de videojuegos con Godot. Creo contenido práctico y aplicado donde explico cómo funcionan las cosas por dentro, usando ejemplos reales, código limpio y buenas prácticas, con el objetivo de ayudar a otros desarrolladores a construir bases sólidas, desarrollar criterio técnico y crear sus propios proyectos con confianza.",
    "about.title": "Sobre Mí",
    "about.description":
      "¡Hola! Soy JuancaDev, un desarrollador apasionado que ama crear aplicaciones y compartir conocimiento con la comunidad.",
    "pagination.first": "Primero",
    "pagination.last": "Último",
    "pagination.prev": "Anterior",
    "pagination.next": "Siguiente",
    "page": "Página",
    "published.on": "Publicado el",
    "updated.on": "Actualizado el",
    "footer.love": "Hecho con ❤️ para la comunidad por JuancaDev",
    "home.work.title": "Trabajo",
    "home.work.current.name": "Dafiti",
    "home.work.current.description": "apoyando el crecimiento de la principal plataforma de comercio electrónico de moda en América Latina.",
    "home.work.previous.name": "Banco de Bogotá",
    "home.work.previous.description": "desarrollando soluciones financieras innovadoras para mejorar la experiencia del cliente.",
    "home.work.last.name": "Canastto",
    "home.work.last.description": "creando y manteniendo servicios backend robustos para una plataforma de comercio electrónico en crecimiento.",
    "home.find.me": "Encuéntrame en",
  },
} as const;
